...
//			output += '<p id="links_' + doc.id + '" class="links"></p>';
			output += '<p>' + snippet + '</p><input type="hidden" id="idCat" value="'+doc.id+'" "'+doc.category+'"/></div>';
			console.log(output);
			return output;
		},
			...
			$(document).on(
					...
					function() {
						...
						$.post("/SolrFlare/loadNewsDescription", data, function(result) {
							var idCat = result.trim().split(" ");
							var data1 = "articleId="+idCat[0]+"&categoryName="+idCat[1];
							$.post("/SolrFlare/UserDocCacheServlet", data1, function(result) {
							});
