...
public class NewsDescription extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			String pattern1 = "(?i)(.*value=\")([\\d]+)(\".*)";
			String docId = description.replaceAll(pattern1, "$2");

			String pattern2 = "(?i)(.*value=\"[\\d]+\"[\\W]+)([\\w]+)([\\W]+.*\">.*)";
			String category = description.replaceAll(pattern2, "$2").replace(";", "").replace("=", "").replace("\"", "");

			session.setAttribute("desc", description);
			//increase category weight here.

			PrintWriter printWriter = response.getWriter();
			printWriter.write(docId+" "+category);
		} catch(Exception e) {
			e.printStackTrace();
		}
	}
